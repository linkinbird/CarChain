 ...
 */
  ...
  function _ownerOf(bytes32 assetId) internal view returns (address) {
    return _holderOf[assetId];
  }

  function takeOwnership(bytes32 assetId) external {
    return _generate(assetId,msg.sender);
  }
  ...
  function _getApprovedAddress(bytes32 assetId) internal view returns (address) {
  ...
  function _isAuthorized(address operator, bytes32 assetId) internal view returns (bool)
  {
    require(operator != 0);
  ...
  function _addAssetTo(address to, bytes32 assetId) internal {
  ...
  function _removeAssetFrom(address from, bytes32 assetId) internal {
    uint256 assetIndex = _indexOfAsset[assetId];
    uint256 lastAssetIndex = _balanceOf(from).sub(1);
    bytes32 lastAssetId = _assetsOf[from][lastAssetIndex];
  ...
  function _clearApproval(address holder, bytes32 assetId) internal {
  ...
  function _generate(bytes32 assetId, address beneficiary) internal {
  ...
  function _destroy(bytes32 assetId) internal {
  ...
  modifier onlyHolder(bytes32 assetId) {
  ...
  modifier isCurrentOwner(address from, bytes32 assetId) {
  ...
  modifier destinataryIsNotHolder(bytes32 assetId, address to) {
